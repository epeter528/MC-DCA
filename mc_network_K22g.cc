...
#pragma omp critical    
    ...
    strcpy(inputstring,argv[8]);
    sscanf(inputstring,"%lg",&reg_H);
    strcpy(inputstring,argv[9]);
    sscanf(inputstring,"%lg",&reg_J); 
    ...
    // Initialize MSA Vector
     ...
     // Set nucleotide indices for each entry in each sequence in a matrix Nxi
      ...
      for(i=0;i<number_of_sequences;i++) {
        ...
        weight[i] = id2/((double)(number_of_sequences));
 ...
 if(excluded[i] == 0) {   
     ...
     for(k=0;k<length;k++) {  
       ...
       for(n=1;n<=q_int;n++) { 
         ...
         if(msa_vec_d[i][k][n] != msa_vec_d[i-1][k][n]) f1[k][n] += weight[i]/B_eff;  
          
         for(l=0;l<length;l++) {  
        
           for(n2=1;n2<=q_int;n2++) {
                    
                if(msa_vec_d[i][k][n] != msa_vec_d[i-1][k][n] && msa_vec_d[i][l][n2] != msa_vec_d[i-1][l][n2]) f12[k][l][n][n2] += weight[i]/pow(B_eff,2);  
...
//#pragma omp parallel for schedule(dynamic)     
  ...
  for(k=0;k<length;k++) {  
   ...
   for(n=1;n<=q_int;n++) {  
      ...
      ran = distribution(generator);    
      ran2 = distribution(generator);
      
      state[k][n] = (ran*h[k][n] + (ran2 - 0.5)*h[k][n])*stepsize_H;
     ...
     // delta_E_N[k][n] = (h[k][n] + state[k][n]);
      ...
      for(l=0;l<length;l++) {  
         ...
         for(n2=1;n2<=q_int;n2++) {      
          ...
          state_2[k][l][n][n2] = (ran*J[k][l][n][n2] + (ran2-0.5)*(J[k][l][n][n2]+h[k][n]*h[l][n2]))*stepsize_J;
         ...
         // delta_J3[k][l][n][n2] = state_2[k][l][n][n2];
...
//#pragma omp parallel for schedule(dynamic)     
  ...
  for(k=0;k<length;k++) {  
   ...
   // printf("%lg\n",1.0/(Z2*temperature)*exp(state[k][n]/temperature));
    ...
    Z2 = Z1 - exp(prob[k][n]) + exp(prob[k][n] + delta_h2[k][n]); 
      
    delta_prob = exp(-((sqrt(pow(1.0/(Z2)*
                       exp(prob[k][n]+delta_h2[k][n]) + reg_H*(h[k][n] + state[k][n]) - f1[k][n],2))
                     - sqrt(pow(1.0/Z1*exp(prob[k][n]) + reg_H*(h[k][n]) - f1[k][n],2))))/temperature);
       
    ran = distribution(generator); 
    
    if(ran <= delta_prob - reg_H*(state[k][n])) { //- reg_H*h[k][n]/accept_1[k][n])) {
       ...
       for(l=0;l<length;l++) {  
         ...
         for(n2=1;n2<=q_int;n2++) { 
           ...
           delta_prob = exp(-(sqrt(pow(1.0/(Z1 - exp(prob[l][n2]) + exp(prob[l][n2] + delta_h2[l][n2]))*
           exp(prob[l][n2] + delta_h2[l][n2]) + 1.0/(Z1 - exp(prob[k][n]) + exp(prob[k][n] + delta_h2[k][n]))*
           exp(prob[k][n] + delta_h2[k][n])*1.0/(Z1 - exp(prob[l][n2]) + exp(prob[l][n2] + delta_h2[l][n2]))*exp(prob[l][n2] + delta_h2[l][n2]) 
           ...
           if(ran <= delta_prob - reg_J*(state_2[k][l][n][n2]))  { 
...
double d_n = (d_a1) / (((double)(length*q_int*number_mc))) + (d_a2)/(double)(length*q_int*length*q_int*number_mc);

std::cout << " Number of accepted steps : h : " << d_a1 << " " << (d_a1) / (((double)(length*q_int*number_mc))) 
<< " J : " << d_a2 << " " << (d_a2)/(double)(length*q_int*length*q_int*number_mc) << " Fraction : " << d_n/2.0 << std::endl;
