    ...
    for(n=1;n<=q_int;n++) { 
      ...
      for(l=0;l<length;l++) {  
        ...
        for(n2=1;n2<=q_int;n2++) {        
            ...
            f12[k][l][n][n2] = 0.5/pow((double)q_int,2);
      ...
      a2[k][n] = (sqrt(pow(1.0/Z2*(exp(delta_h2[k][n])) - f1[k][n],2)));
       ...
       for(l=0;l<length;l++) {
         ...
         for(n2=1;n2<=q_int;n2++) {
             ...
             a1[k][n] += sqrt(pow(1.0/pow(Z1,2)*(exp(prob[k][n])*Z1 - exp(prob[l][n2])*Z1) + 1.0/pow(Z1,2)*exp(prob[l][n2]*exp(prob[k][n])) - f12[k][l][n][n2],2));
             a2[k][n] += sqrt(pow(1.0/pow(Z2,2)*(exp(delta_h2[k][n])*Z2 - exp(delta_h2[l][n2])*Z2) + 1.0/pow(Z2,2)*exp(delta_h2[l][n2]*exp(delta_h2[k][n])) - f12[k][l][n][n2],2));
	 }
       }
    ...
    if(ran <= delta_prob) { //- reg_H*h[k][n]/accept_1[k][n])) {
       ...
       for(l=0;l<length;l++) {  
        ...
        //   + reg_J*J[k][l][n][n2] - f12[k][l][n][n2],2)))/temperature);
           ...
           delta_prob = exp(-(sqrt(pow(1.0/pow(Z2,2)*(exp(delta_h2[k][n])*Z2 - exp(delta_h2[l][n2])*Z2) + 1.0/pow(Z2,2)*exp(delta_h2[l][n2]*exp(delta_h2[k][n])) - f12[k][l][n][n2],2)) - 
           sqrt(pow(1.0/pow(Z1,2)*(exp(prob[k][n])*Z1 - exp(prob[l][n2])*Z1) + 1.0/pow(Z1,2)*exp(prob[l][n2]*exp(prob[k][n])) - f12[k][l][n][n2],2)))/temperature)*Z3/Z4*exp(-sqrt(pow(state_2[k][l][n][n2],2)));
